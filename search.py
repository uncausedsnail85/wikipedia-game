...
def heuristic(pageTitle, destinationTitle):
    return 0 # temp
...
def wikiGameAStar(initialPageTitle, destinationPageTitle, heuristic):
    ...
    frontier.put((heuristic(initialNode[0], destinationPageTitle), initialNode))
    # explored set
    explored = [] #List of titles (string)
        ...
        for succ in succList:
            if (succ not in explored):
                ...
                if (succNode[0] not in explored):
                    frontier.put((succPathCost + heuristic(succNode[0], destinationPageTitle), succNode))
